...
import { Mail, Lock, LogIn, User, ShieldCheck, Key, X, Send, CheckCircle, UserPlus } from 'lucide-react';
...
export default function Login() {
  ...
  // Şifremi Unuttum Modal Stateleri
  const [isForgotOpen, setIsForgotOpen] = useState(false);
  const [forgotEmail, setForgotEmail] = useState('');
  const [forgotLoading, setForgotLoading] = useState(false);
  const [forgotMessage, setForgotMessage] = useState({ type: '', text: '' });

  // 🌟 YENİ: Kayıt Ol Modal Stateleri
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api.post('/Auth/forgot-password', { email: forgotEmail, resetUrl }) as any;
  ...
  const handleRegisterInitiate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/Auth/register-init', { email: registerEmail }) as any;
      setRegisterMessage({ 
        type: 'success', 
        text: response.message || response.data?.message || t('Register_SuccessMessage') 
      });
    } catch (err: any) {
      const backendMessage = err.response?.data?.message || err.message;
      setRegisterMessage({ type: 'error', text: backendMessage || t('Register_ErrorMessage') });
    } finally {
  ...
  const buttonClass = isStudent ? "from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 shadow-pink-500/30" : "from-blue-600 to-purple-600 hover:from-blue-700 hover:to-blue-700 shadow-blue-500/30";
        ...
        <div className="text-center pt-2 border-t border-gray-100">
          <p className="text-sm text-gray-500">
            ...
            <button 
              ...
              className={`font-semibold hover:underline text-${themeColor}-600 hover:text-${themeColor}-800`}
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity duration-300">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl relative">
            <div className="absolute top-4 right-4">
              <button onClick={() => setIsForgotOpen(false)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
            </div>
            ...
            {forgotMessage.type !== 'success' && (
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Mail className="h-5 w-5 text-gray-400" /></div>
                  <input type="email" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className={`w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 outline-none focus:ring-${themeColor}-500 focus:border-${themeColor}-500`} placeholder="ornek@univibe.com" required />
                </div>
                <button type="submit" disabled={forgotLoading} className={`w-full bg-gradient-to-r text-white font-medium py-2.5 rounded-lg flex items-center justify-center gap-2 shadow-md disabled:opacity-70 ${buttonClass}`}>
                  {forgotLoading ? t('Forgot_Sending') : <><Send className="w-4 h-4" /> {t('Forgot_SendButton')}</>}
                </button>
              </form>
            )}
            {forgotMessage.type === 'success' && (
              <button onClick={() => setIsForgotOpen(false)} className="w-full mt-2 bg-gray-100 text-gray-700 font-medium py-2.5 rounded-lg hover:bg-gray-200">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity duration-300">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl relative">
            ...
            <div className="text-center space-y-3 mb-6 mt-2">
              ...
              <h3 className="text-xl font-bold text-gray-800">{t('Register_ModalTitle')}</h3>
              <p className="text-sm text-gray-500 px-2">{t('Register_ModalDesc')}</p>
            </div>
            {registerMessage.text && (
              <div className={`p-3 rounded-lg text-sm mb-4 flex items-start gap-2 ${registerMessage.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'}`}>
                {registerMessage.type === 'success' ? <CheckCircle className="w-5 h-5 shrink-0" /> : <X className="w-5 h-5 shrink-0" />}
            ...
            {registerMessage.type !== 'success' && (
              <form onSubmit={handleRegisterInitiate} className="space-y-4">
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Mail className="h-5 w-5 text-gray-400" /></div>
                  <input type="email" value={registerEmail} onChange={(e) => setRegisterEmail(e.target.value)} className={`w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 outline-none focus:ring-${themeColor}-500 focus:border-${themeColor}-500`} placeholder="ornek@univibe.com" required />
                </div>
                <button type="submit" disabled={registerLoading} className={`w-full bg-gradient-to-r text-white font-medium py-2.5 rounded-lg flex items-center justify-center gap-2 shadow-md disabled:opacity-70 ${buttonClass}`}>
                  {registerLoading ? t('Register_Sending') : <><Send className="w-4 h-4" /> {t('Register_SendButton')}</>}
                </button>
              </form>
            )}
            {registerMessage.type === 'success' && (
              <button onClick={() => setIsRegisterOpen(false)} className="w-full mt-2 bg-gray-100 text-gray-700 font-medium py-2.5 rounded-lg hover:bg-gray-200">
                {t('Forgot_CloseButton')}
              </button>
            )}
          </div>
